...
const GalleryKeyboardSupport = ({ onNext, onPrev, onClose }: { onNext: () => void, onPrev: () => void, onClose: () => void }) => {
  useEffect(() => {
...
export default function App() {
  ...
  const [selectedFolder, setSelectedFolder] = useState<OdysseiaData['gallery'][0] | null>(null);
  ...
  const currentFolderPhotos = selectedFolder?.photos || [];
  const currentPhotoIndex = selectedPhoto ? currentFolderPhotos.indexOf(selectedPhoto) : -1;

  const navigateGallery = (direction: 'prev' | 'next') => {
    if (currentFolderPhotos.length === 0) return;
    let nextIndex = direction === 'next' ? currentPhotoIndex + 1 : currentPhotoIndex - 1;
    
    if (nextIndex >= currentFolderPhotos.length) nextIndex = 0;
    if (nextIndex < 0) nextIndex = currentFolderPhotos.length - 1;
    
    setSelectedPhoto(currentFolderPhotos[nextIndex]);
  };
  ...
  return (
    ...
    >
      ...
      <Scrollytelling 
        ...
        zoomFactor={1.35}
        wrapperHeight="350vh"
        frameUrlTemplate={(index) => `/frames/ezgif-frame-${String(index).padStart(3, '0')}.jpg`}
      >
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mb-32 -mt-4 md:-mt-8 relative z-30">
        <div className="bg-[#1a1611]/60 backdrop-blur-2xl rounded-3xl border border-white/5 shadow-[0_30px_60px_rgba(0,0,0,0.6)] overflow-hidden">
                  ...
                  <div className="relative rounded-[28px] overflow-hidden aspect-video lg:aspect-[4/5] border border-white/10">
                    {data.general.aboutMedia?.type === 'video' ? (
                      <video 
                        ...
                        className={`w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 scale-105 group-hover:scale-100 object-${data.general.aboutMedia.position || 'center'}`} 
              ...
              {data.gallery.map((edition) => (
                <div key={edition.id} className="group relative bg-[#0d0a06]/40 rounded-2xl overflow-hidden border border-white/5 hover:-translate-y-2 flex flex-col h-full shadow-lg transition-all duration-500">
                  <div className="aspect-[4/3] w-full overflow-hidden relative">
                    <img className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700" src={edition.cover} alt={edition.title} />
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="font-headline text-2xl text-primary">{edition.title}</h3>
                      <span className="font-label text-secondary/60 text-[10px] tracking-widest uppercase">{edition.year}</span>
                    </div>
                    <button onClick={() => setSelectedFolder(edition)} className="mt-auto flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-primary/10 border border-primary/30 text-primary font-label text-[10px] uppercase tracking-widest hover:bg-primary/20 transition-colors">
          ...
          <div className="p-8 md:p-16 lg:p-20 py-24" id="ingressos">
            ...
            <div className={`flex flex-wrap justify-center gap-12 ${data.tickets.filter(tier => tier.isActive).length === 3 ? 'md:grid md:grid-cols-3' : ''}`}>
              {data.tickets.filter(tier => tier.isActive).map(tier => (
                <div key={tier.id} className={`relative p-10 bg-[#0d0a06]/40 rounded-3xl border transition-all duration-500 shadow-xl ${tier.status === 'ESGOTADO' ? 'opacity-40 grayscale' : ''} ${tier.highlighted ? 'border-primary/50 shadow-[0_0_50px_rgba(239,159,39,0.1)] scale-105 z-10' : 'border-white/5'}`}>
                  ...
                  <div className={`text-4xl font-headline text-primary mb-12 ${tier.status === 'ESGOTADO' ? 'line-through opacity-50' : ''}`}>
                    {tier.price}{tier.price.includes('R$') && tier.price.length > 3 && <span className="text-sm font-label opacity-60">,00</span>}
                  </div>
      ...
      {selectedFolder && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 md:p-8" aria-modal="true">
          <div className="fixed inset-0 bg-background/98 transition-opacity" onClick={() => setSelectedFolder(null)}></div>
          <div className="relative bg-[#1a1611] border border-white/10 rounded-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col z-10 shadow-[0_0_100px_rgba(0,0,0,0.8)]">
            <div className="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-[#0d0a06]/80 sticky top-0 z-20">
              <div className="flex flex-col">
                <h3 className="text-3xl font-headline text-primary">{selectedFolder.title.toUpperCase()}</h3>
                <p className="text-xs font-label text-white/40 uppercase tracking-widest mt-1">Galeria de Fotos • {selectedFolder.photos.length} imagens</p>
              </div>
              <button onClick={() => setSelectedFolder(null)} className="text-white/40 hover:text-primary transition-all p-3 rounded-full hover:bg-primary/10">
              ...
              {selectedFolder.photos.length > 0 ? (
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 md:gap-4">
                  {selectedFolder.photos.map((photo, index) => (
                    <div key={index} onClick={() => setSelectedPhoto(photo)} className="group aspect-square relative cursor-zoom-in rounded-lg overflow-hidden border border-white/5 hover:border-primary/50 transition-all">
                      <img src={photo} className="w-full h-full object-cover grayscale hover:grayscale-0 hover:scale-110 transition-all duration-500" alt={`Edition Photo ${index}`} />
                ...
                <div className="flex flex-col items-center justify-center py-24 text-center text-white/40">
                  <span className="material-symbols-outlined text-7xl mb-4">image_not_supported</span>
                  <p className="font-label uppercase tracking-widest text-sm">Ainda não há fotos nesta pasta.</p>
                </div>
              )}
            </div>
          </div>
      ...
      {selectedPhoto && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-0" aria-modal="true">
          <GalleryKeyboardSupport 
            onNext={() => navigateGallery('next')} 
            onPrev={() => navigateGallery('prev')} 
            onClose={() => setSelectedPhoto(null)} 
          />
          <div className="fixed inset-0 bg-black/95 transition-opacity" onClick={() => setSelectedPhoto(null)}></div>
          <button onClick={() => setSelectedPhoto(null)} className="absolute top-6 right-6 z-30 text-white/80 hover:text-white p-3 rounded-full bg-black/50 hover:bg-black/80 transition-all backdrop-blur-sm">
            <span className="material-symbols-outlined text-3xl">close</span>
          </button>
            <div className="flex-1 flex flex-col items-center justify-center p-4 relative group/photo">
              {/* Photo Navigation */}
              <button 
                onClick={(e) => { e.stopPropagation(); navigateGallery('prev'); }}
                className="absolute left-4 top-1/2 -translate-y-1/2 z-10 w-16 h-16 rounded-full bg-black/20 backdrop-blur-xl border border-white/10 text-white flex items-center justify-center opacity-0 group-hover/photo:opacity-100 transition-all hover:bg-black/40"
              >
                <span className="material-symbols-outlined text-4xl">chevron_left</span>
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); navigateGallery('next'); }}
                className="absolute right-4 top-1/2 -translate-y-1/2 z-10 w-16 h-16 rounded-full bg-black/20 backdrop-blur-xl border border-white/10 text-white flex items-center justify-center opacity-0 group-hover/photo:opacity-100 transition-all hover:bg-black/40"
              >
              ...
              <img 
                src={selectedPhoto} 
                alt="Galeria Odysseia" 
                className="max-w-[90vw] max-h-[85vh] object-contain rounded-lg shadow-2xl"
              />
              <div className="mt-4 flex items-center gap-4">
                <p className="text-secondary/60 text-xs uppercase tracking-widest">{currentPhotoIndex + 1} / {currentFolderPhotos.length}</p>
                <button 
                  onClick={() => {
                    const l = document.createElement('a');
                    l.href = selectedPhoto;
                    l.download = `odysseia-${selectedFolder?.title}-${currentPhotoIndex}.jpg`;
                    l.click();
                  }}
      ...
      <footer className="bg-[#1a1200]/90 border-t border-[#ef9f27]/10">
        <div className="w-full py-16 px-8 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          ...
          <button onClick={() => setIsLoginOpen(true)} className="bg-primary text-on-primary font-label font-bold py-2 px-4 rounded-lg hover:shadow-[0_0_20px_rgba(239,159,39,0.4)] transition-all">Login</button>
          <div className="flex gap-8">
            <a className="text-[#ffb869]/60 hover:text-[#ef9f27] transition-colors" href="#"><span className="material-symbols-outlined">brand_awareness</span></a>
            <a className="text-[#ffb869]/60 hover:text-[#ef9f27] transition-colors" href="#"><span className="material-symbols-outlined">photo_camera</span></a>
            <a className="text-[#ffb869]/60 hover:text-[#ef9f27] transition-colors" href="#"><span className="material-symbols-outlined">music_note</span></a>
          </div>
          <div className="flex flex-col items-center md:items-end gap-2">
            <div className="flex gap-6 mb-4">
              <a className="text-xs font-label uppercase tracking-widest text-[#ffb869]/60 hover:text-[#ef9f27] transition-colors" href="#">Instagram</a>
              <a className="text-xs font-label uppercase tracking-widest text-[#ffb869]/60 hover:text-[#ef9f27] transition-colors" href="#">Contato</a>
              <a className="text-xs font-label uppercase tracking-widest text-[#ffb869]/60 hover:text-[#ef9f27] transition-colors" href="#">Termos</a>
            </div>
